...
- Added retained oracle corpus contracts and correlation: normalized oracle expectation artifacts are read, byte-count/digest verified, decoded and compared against native typed results before qualification evidence can be created.
...
- Added Xcircuite persistence of typed DFT results, release-time SHA-256/byte-count verification for DFT and downstream artifacts, an immutable eligibility artifact and a blocked review/resume artifact with stable run and design-digest identity.
