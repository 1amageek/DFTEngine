...
| Scan insertion | Implemented | Digest-verified native canonical transformation plus retained OpenROAD pre/post Verilog and DEF `SCANCHAINS` import, explicit clock/reset/control binding, process-mapped compression helpers, validator pass and design diff | DFTEngine tests + CLI fixture + OpenROAD import success/tamper/order/top-level-endpoint/completeness tests + 2,048-chain performance regression | OpenROAD adapter behavior verified with retained fixtures; hosted real-tool execution remains unqualified |
